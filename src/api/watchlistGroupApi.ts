...
export const getMoviesByCategory = async (
  ...
  const response = await fetch(
    `${API_BASE_URL}/watchlists/movies-by-group/${userId}/${groupId}`
  );
  if (!response.ok) {
...
export const addWatchlistGroup = async (
  categoryName: string
): Promise<WatchlistGroup> => {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/watchlists/add-directly`,
      null,
      {
...
export const editWatchlistGroup = async (
  groupId: number,
  newName: string
): Promise<void> => {
  try {
...
export const createOrGetWatchlistGroup = async (
  name: string
): Promise<WatchlistGroup> => {
  try {
    const response = await axios.post(
      `${API_BASE_URL}/watchlists/add-indirectly`,
      null,
      {
        params: { name },
      }
